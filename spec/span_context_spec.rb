...
RSpec.describe 'SpanContext' do
  ...
  describe '#initialize' do
    it 'sets attributes' do
      sc = SpanContext.new('some-trace-id', 'some-span-id', true, {item: 'val'})
    ...
    it 'defaults sampled and baggae' do
      sc = SpanContext.new('some-trace-id', 'some-span-id')
  ...
  describe '#foreach_baggage_item' do
    before(:each) do
      @sc = SpanContext.new('some-trace-id', 'some-span-id', true, {item1: 'val1', item2: 'val2'})
    end
